...
class SolutionCreate(BaseModel):
    contest: str; platform: str; problem: str; difficulty: str
    language: str; topics: List[str] = []; code: str
    explanation: str = ""; admin_pin: str

class SolutionUpdate(BaseModel):
    contest: Optional[str]=None; platform: Optional[str]=None
    problem: Optional[str]=None; difficulty: Optional[str]=None
    language: Optional[str]=None; topics: Optional[List[str]]=None
    code: Optional[str]=None; explanation: Optional[str]=None
    admin_pin: str
...
def to_out(sol: Solution) -> dict:
    ...
    return {
        "id": sol.id, "contest": sol.contest, "platform": sol.platform,
        "problem": sol.problem, "difficulty": sol.difficulty,
        "language": sol.language, "topics": topics,
        "code": sol.code, "explanation": sol.explanation,
    }
...
# ─── App ──────────────────────────────────────────────────────────────────────
app = FastAPI(title="ContestHive API", lifespan=lifespan)
app.add_middleware(
    CORSMiddleware,
...
# ─── Routes ───────────────────────────────────────────────────────────────────
@app.get("/")
def health(): return {"status": "ok", "message": "ContestHive API running 🚀"}

@app.get("/api/solutions")
def get_solutions(
    platform: Optional[str]=Query(None), difficulty: Optional[str]=Query(None),
    language: Optional[str]=Query(None),  search: Optional[str]=Query(None),
    db: Session=Depends(get_db),
):
    q = db.query(Solution)
    if platform:   q = q.filter(Solution.platform.ilike(f"%{platform}%"))
    if difficulty: q = q.filter(Solution.difficulty.ilike(f"%{difficulty}%"))
    if language:   q = q.filter(Solution.language.ilike(f"%{language}%"))
    if search:
        q = q.filter(Solution.problem.ilike(f"%{search}%") |
                     Solution.contest.ilike(f"%{search}%") |
                     Solution.explanation.ilike(f"%{search}%"))
    return [to_out(s) for s in q.all()]

@app.get("/api/solutions/ai-search")
def ai_search(q: str=Query(...), db: Session=Depends(get_db)):
    if not q.strip():
    ...
    results = (
        ...
        .order_by(Solution.embedding.op("<=>")(emb))
        .limit(5)
        .all()
    )
    return [to_out(s) for s in results]
...
def get_solution(solution_id: int, db: Session=Depends(get_db)):
    ...
    if not sol: raise HTTPException(status_code=404, detail="Not found")
    return to_out(sol)
...
def create_solution(data: SolutionCreate, db: Session=Depends(get_db)):
    ...
    emb = get_embedding(f"{data.problem} {data.explanation} {' '.join(data.topics)}")
    sol = Solution(
        contest=data.contest, platform=data.platform, problem=data.problem,
        difficulty=data.difficulty, language=data.language,
        topics=json.dumps(data.topics), code=data.code,
        explanation=data.explanation, embedding=emb,
    )
...
def update_solution(solution_id: int, data: SolutionUpdate, db: Session=Depends(get_db)):
    ...
    sol = db.query(Solution).filter(Solution.id==solution_id).first()
    if not sol: raise HTTPException(status_code=404, detail="Not found")
    if data.contest:     sol.contest = data.contest
    if data.platform:    sol.platform = data.platform
    if data.problem:     sol.problem = data.problem
    if data.difficulty:  sol.difficulty = data.difficulty
    if data.language:    sol.language = data.language
    if data.topics:      sol.topics = json.dumps(data.topics)
    if data.code:        sol.code = data.code
    if data.explanation: sol.explanation = data.explanation
    try:    topics = json.loads(sol.topics)
    except: topics = []
    sol.embedding = get_embedding(f"{sol.problem} {sol.explanation} {' '.join(topics)}")
    db.commit(); db.refresh(sol)
    return to_out(sol)

@app.delete("/api/solutions/{solution_id}")
def delete_solution(solution_id: int, admin_pin: str=Query(...), db: Session=Depends(get_db)):
...
def seed_data(admin_pin: str=Query(...), db: Session=Depends(get_db)):
    ...
    count = 0
    for s in SEEDS:
        emb = get_embedding(f"{s['problem']} {s['explanation']} {' '.join(s['topics'])}")
        db.add(Solution(
            ...
            explanation=s["explanation"], embedding=emb,
        ))
        count += 1
    db.commit()
    return {"message": f"Seeded {count} solutions!"}
